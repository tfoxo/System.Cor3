...
//            }catch{}
        ...
        {
            Filter="Feed Text|*.txt;*.text|All Files|*"
        };
        OpenFileDialog ofd_AllXml = new OpenFileDialog()
        {
            Filter="Xml File|*.xml;*.rss*.opml*.feed|All Files|*"
        };
        OpenFileDialog ofd_opml = new OpenFileDialog()
        {
            Filter="OPML File|*.opml;*.xml|All Files|*"
        };
        Timer SelectorTimer = new Timer()
        {
            Interval = 300
        };
        SaveFileDialog sfd_xml = new SaveFileDialog()
        {
            Filter="Xml File|*.xml;*.rss*.opml*.feed|All Files|*"
        };
        SaveFileDialog sfd_txt = new SaveFileDialog()
        {
            Filter="Feed Text|*.txt;*.text|All Files|*"
        };
        ...
        /// </summary>
        string CurrentFeedTextFileName
        {
            get;
            set;
        }
        ...
        /// </summary>
        Dictionary<string,FeedListItem> FeedDictionary
        {
            get;
            set;
        }
        ...
        /// </summary>
        Dictionary<string,FeedParser> XmlList
        {
            get;
            set;
        }
        ...
        public MainForm()
        {
            ...
            mrMenu.MenuItems.Add(
               "More Actions",
               new MenuItem[]{
                   new MenuItem("Load OPML",Event_TryLoadOPML),
               }
               );
//            this.Controls.Add(mrMenu);
            ...
            SelectorTimer.Tick += Event_TimedSelection; // not used
...
//            this.refreshSelectedToolStripMenuItem.Click += new System.EventHandler(this.Event_LoadText);
                        ...
                        XmlList[k].ActionCompleted = delegate
                        {
                            try
                            {
                                ...
                                System.Diagnostics.Debug.Print(
                                    "DownloadUrl Completed Loading/Parsing Child elements"
                                );
        ...
        void GetFeedsFile(string fileName)
        {
...
//                    );
        ...
        {
            if (SelectorTimer.Enabled) return;
            //ItemSelector(listBox1.SelectedItem as NodeInfo);
        }
        ...
        void GotOpml(OpmlDocument doc)
        {
            ...
            {
                if (node.XmlUrl==null) continue;
                string nodeName = FeedDictionary.ContainsKey(node.Text) ?
                    string.Format("{0} ({1})",node.Text,indx++):
                    node.Text
                    ;
