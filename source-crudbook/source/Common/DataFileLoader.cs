...
namespace crudbook
{
	...
	/// </summary>
	public class DataFileLoader : INotifyPropertyChanged
	{
		#region Commands
		...
		static public readonly OpenFileDialog OFD_DatabaseFile = new OpenFileDialog() {
			Filter = "SQlite Database (*.db3)|*.db3|" + "SQLite Database (*.db)|*.db|" + "SQlite Database (*.dbx)|*.dbx|" + "SQLite3 Database (*.sqlite)|*.sqlite|" + "All files|*"
		};
		...
		public void Load()
		{
			if (!OFD_DatabaseFile.ShowDialog().Value) {
				return;
			}
