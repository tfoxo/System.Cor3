...
namespace System.Cor3.Forms
{
	...
	public class ToolStripTextBoxNumber : System.Windows.Forms.ToolStripTextBox
	{
		...
		internal string strfmt = def_strfmt;
		internal FloatPoint pos_down,pos_move,pos_prev;
		internal MouseButtons ButtonFilter = MouseButtons.Left;
		internal bool dwn;
		internal float wedgel = 10, wedges=100;
		internal float min = def_min,max=def_max;
		...
		{
			pos_move = new FloatPoint(e.X,e.Y);
			switch (e.Button)
			{
				case MouseButtons.Left:
					if ( dwn & ( pos_move != null ) )
					{
						FloatPoint x = pos_prev-pos_move;
						Value += ( ( x * ( 1f/ wedgel ) ) as FloatPoint).Y*.2f;
						pos_prev = pos_move;
					}
					else pos_down = null;
					break;
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			switch (e.Button)
			{
				case MouseButtons.Left: pos_down = new FloatPoint(e.X,e.Y); pos_prev = pos_down; break;
			}
			Global.stat("do");
			dwn = true;
			base.OnMouseDown(e);
